...
function hasRole(principal: AuthenticatedPrincipal, expected: 'super-admin' | 'school-admin'): boolean { return principal.roles.includes(expected); }
...
export class TenantSchoolService {
  ...
  async createTenant(request: ContextualRequest, body: unknown) {
    const principal = requirePrincipal(request); if (!hasRole(principal, 'super-admin')) throw new ForbiddenException();
  ...
  async getTenant(request: ContextualRequest, id: string) {
    ...
    if (!hasRole(principal, 'super-admin') && (!hasRole(principal, 'school-admin') || principal.tenantId !== record.id)) throw new NotFoundException(); return record;
  }
  ...
  async updateTenant(request: ContextualRequest, id: string, body: unknown) {
    ...
    if (!hasRole(principal, 'super-admin')) throw new NotFoundException();
  ...
  async createSchool(request: ContextualRequest, body: unknown) {
    ...
    if (!hasRole(principal, 'super-admin') && (!hasRole(principal, 'school-admin') || principal.tenantId !== requestedTenant)) throw new NotFoundException();
  ...
  async getSchool(request: ContextualRequest, id: string) {
    ...
    if (!hasRole(principal, 'super-admin') && (!hasRole(principal, 'school-admin') || principal.tenantId !== record.tenantId)) throw new NotFoundException(); return record;
  }
  ...
  async updateSchool(request: ContextualRequest, id: string, body: unknown) {
    ...
    if (!hasRole(principal, 'super-admin') && (!hasRole(principal, 'school-admin') || principal.tenantId !== current.tenantId)) throw new NotFoundException();
