...
async function persist<T>(operation: () => Promise<T>): Promise<T> { try { return await operation(); } catch (error) { const code = (error as { code?: string }).code; if (code === '23505') throw new ConflictException(); if (code === '23503' || code === '23514') throw new BadRequestException(); if (code === 'TENANT_LIFECYCLE_DENIED') throw new NotFoundException(); throw error; } }
...
export class TenantSchoolService {
  ...
  async getTenant(context: TenantSchoolAuthorizationContext, id: string) {
    ...
    const record = await this.repository.getTenant(id, scopedTenant); if (!record || !this.policy.canAccessTenant(context, record.id)) throw new NotFoundException(); if (!this.policy.isSuperAdmin(context) && record.status !== 'active') throw new NotFoundException(); return record;
  }
