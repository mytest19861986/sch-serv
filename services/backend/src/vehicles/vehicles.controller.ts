...
import type { VehiclesAuthorizationContext } from './vehicles.policy.js';

@Controller('vehicles')
@UseGuards(AuthGuard)
export class VehiclesController {
  constructor(private readonly service: VehiclesService) {}
  private context(request: ContextualRequest): VehiclesAuthorizationContext { if (!request.principal) throw new UnauthorizedException(); return { principal: request.principal, correlationId: request.correlationId ?? 'unavailable' }; }
